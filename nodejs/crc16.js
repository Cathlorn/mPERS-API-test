...
function crc16() {}
  ...
  crc16.hdlc_init_fcs16 = 0;
  crc16.hdlc_good_fcs16 = 0x0DDD;
  crc16.ccitt_crc16_table = [ 0x0000, 0x1021, 0x2042, 0x3063,
   ...
   crc16.test = function() {
    ...
    var testPacket = [ 0x42,0x0A,0x01,0x08,0xA1,0xFF,0xFF,0xFF,0xFF,
                       0xAA,0x7D,0x01,0x00,0x01,0x00,0xA0,0x00,0xE6,0x00,0x52,0x00];

    var crc = crc16.hdlcFcs16(crc16.hdlc_init_fcs16, testPacket,0, 21);

    if (crc != crc16.hdlc_good_fcs16)
    {
        log('CRC16 doesn\'t match.');
        log('calculated: '+crc + ' expected: ' + crc16.hdlc_good_fcs16);
    }
    else
    {
        log('CRC16 matches!');
  ...
  crc16.crcByte = function(fcs, c) {
     fcs = ((crc16.ccitt_crc16_table[(fcs >> 8 ^ c) & 0xff] ^ (fcs << 8))&0xffff);
     return fcs;
  }

  crc16.hdlcFcs16 = function(fcs, cp, offset, len) {

     for (var i = 0; i < len; i++)
         fcs = crc16.crcByte(fcs, cp[offset + i]);

     return (fcs);
  }

  crc16.calculateCrc = function(cp, offset, len) {
      return crc16.hdlcFcs16(crc16.hdlc_init_fcs16, cp, offset, len);
  }
